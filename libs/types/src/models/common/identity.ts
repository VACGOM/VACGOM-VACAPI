...
export class Identity {
  ...
  constructor(birthDate: LocalDate, rnnSecret?: string) {
    if (rnnSecret && rnnSecret.length != 7)
      throw new InvalidRnnSecretException();
  ...
  static fromPartialRnnString(partialRnn: string): Identity {
    ...
    const matches = partialRnn.match(/(\d{6})-*(\d)/g);
    if (!matches || matches.length != 2) throw new InvalidRnnSecretException();

    const bornIn1900s = this.isBornIn1900s(matches[1]);

    const birthDate = LocalDate.parse(
      `${bornIn1900s ? '19' : '20'}${matches[0]}`
    );

    return new Identity(birthDate);
  }
}
